...
**1. 포커**
   ...
   리얼함을 위해 랜덤으로 뽑힌 카드의 순서를 저장하기 위한 별도의 카드 배열을 두는 것은 불가피 하기 때문에 카드 배열을 순회하며 족보를 판별하는 방식보다 64비트만큼의 추가 메모리가 더 필요하다는 단점이 있지만,  
   카드의 존재 유무를 확인하기 위해 뽑은 카드 배열을 순회할 필요 없이 비트 연산으로 O(1) 시간에 확인할 수 있어 매우 빠르게 족보를 판별할 수 있다는 장점이 있다.  
   족보 판별에 있어 카드의 존재 유무를 확인하는 연산이 많이 호출되기 때문에 비트마스킹 기법으로 구현하는 방식을 채택하였다.  

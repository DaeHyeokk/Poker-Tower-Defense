...
**1. 포커**
   - 족보 판별  
   카드의 갯수가 총 **52장**이며 중복이 없다는 특성을 이용하여 **64비트 자료형**인 long 타입의 변수에 뽑은 카드의 정보를 저장하는 **비트마스킹 기법**으로 구현하였다.  
   리얼함을 위해 랜덤으로 뽑힌 카드의 순서를 저장하기 위한 카드 배열을 두는 것은 불가피 하기 때문에 카드 배열을 순회하며 족보를 판별하는 방식보다 64비트만큼의 추가 메모리가 더 필요하다는 단점이 있지만, 카드의 존재 유무를 확인하기 위해 뽑은 카드 배열을 순회할 필요 없이 비트 연산으로 O(1) 시간에 알 수 있기 때문에 매우 빠르게 족보를 판별할 수 있다는 장점이 있다.
